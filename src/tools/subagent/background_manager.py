...
class BackgroundSubagentManager:
    ...
    async def _execute_in_background(
        ...
        try:
            ...
            result = await runner.run(self._tasks[task_id].prompt)
            ...
            async with self._lock:
                ...
                self._notification_queue.append({
                    "task_id": task_id,
                    "status": "completed",
                    "subagent_name": self._tasks[task_id].subagent_name,
            ...
            async with self._lock:
                ...
                self._notification_queue.append({
                    "task_id": task_id,
                    "status": "error",
                    "subagent_name": self._tasks[task_id].subagent_name if task_id in self._tasks else "unknown",
