...
Controls the agent execution loop: iteration limits, state tracking,
termination conditions, tool call history, retry and context management.
"""
...
class IdleException(Exception):
    ...
    Unlike error exceptions, idle does not mark work items as failed.
    """
...
class AgentLoop:
    ...
    def __init__(
        ...
        on_confirmation_check: SubagentRunner-style callback
            (response, stop_reason) -> Optional[bool]; True = confirmed complete,
            False = not confirmed, None = skip.
        """
    ...
    def compress_context(self, keep_recent: int = 10) -> int:
        """Compress context by keeping only the most recent messages.
    ...
    async def start_iteration(self) -> bool:
        ...
        if not self.can_continue():
            ...
            logger.info(f"Loop cannot continue: {reason}")
        ...
        if self.work_item_source:
            ...
            if self._failed_work_item and self._retry_count < self._max_retries_per_task:
                ...
                self._retry_count += 1
                logger.info(f"Retrying failed work item: id={self._current_work_item.id}, retry={self._retry_count}")
                await self.callbacks.emit(
                ...
                if self._failed_work_item and self._retry_count >= self._max_retries_per_task:
                    logger.info(f"Max retries reached for item {self._failed_work_item.id}, marking as failed and getting next")
                ...
                self._retry_count = 0
                logger.info(f"Starting work item: id={self._current_work_item.id}, description={self._current_work_item.description[:50]}...")
                await self.callbacks.emit(
                    LoopEvent.WORK_ITEM_START,
                    item=self._current_work_item
                )
    ...
    async def execute_with_tools(
        ...
        execute_fn returns (response, tool_calls[, stop_reason]);
    ...
    async def run(
        ...
        execute_fn returns (response, tool_calls); returns the final response.
        """
        start_time = time.time()
        response = ""
        tool_calls = []
        ...
        while self.should_continue:
            ...
            try:
                ...
                logger.info(f"IdleException caught: {e.message if e.message else 'enter idle state'}")
                return ""
            except Exception as e:
                logger.error(f"Unexpected exception in execute_with_tools: {e}")
                raise
